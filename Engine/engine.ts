export class Camera {
    ...
    drawScene(){
        ...
        let currentLayer = 0;
        while (currentLayer <= this.maxLayer) {
    ...
    addDrawable(add: Drawable){
        ...
        if (add.layer > this.maxLayer){
            this.maxLayer = add.layer;
        }
    }
...
export class Box implements Drawable, Physical{
    ...
    constructor(x: number, y: number, width: number, height: number, color: Color, layer: number){
        this.x = x;
        this.y = y;
        this.width = width;
        this.height = height;
...
export class Engine {
    ...
    addCamera(camera: Camera): Engine {
        ...
        if (this.cameras.length == 1) {
            ...
            this.currentCamera.setCanvas(this.canvas);
        }
        return this;
    }
